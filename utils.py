...
def load_google_drive_excel(file_url):
    ...
    try:
        ...
        required_columns = ['Ticker', 'Papan Pencatatan']
        if not all(col in df.columns for col in required_columns):
            raise ValueError(f"Kolom {required_columns} harus ada di file Excel")
            
        return df
        
    except requests.exceptions.RequestException as e:
        print(f"Gagal mengakses file: {e}")
        return None
...
def get_stock_data(ticker, end_date):
    ...
    try:
        # Format ticker untuk pasar Indonesia
...
def compute_mfi(df, period=14):
    ...
    pos_mf = pos_series.rolling(period).sum()
    neg_mf = neg_series.rolling(period).sum()
...
def identify_significant_swings(df, window=60, min_swing_size=0.05):
    """Identifikasi swing high dan low yang signifikan"""
    highs = df['High']
    lows = df['Low']
...
def calculate_vwap(df):
    """Hitung Volume Weighted Average Price (VWAP)"""
    tp = (df['High'] + df['Low'] + df['Close']) / 3
    vwap = (tp * df['Volume']).cumsum() / df['Volume'].cumsum()
    return vwap

def find_psychological_levels(close_price):
    """Temukan level psikologis terdekat"""
