...
try:
    ...
    st.error("⚠️ Library `yfinance` tidak ditemukan. Silakan instal dengan: `pip install yfinance`")
    st.stop()
...
def interpret_bollinger_bands(price, upper_band, middle_band, lower_band, volume, avg_volume, hist):
    """
    Interpretasi Bollinger Bands yang komprehensif
    """
    ...
    # Analisis Reversal
    reversal = ""
    if current_price > current_upper and hist.iloc[-1] < 0:
        reversal = "🔄 **Potensi Reversal Bearish** - Harga melampaui upper band dengan konfirmasi MACD"
    elif current_price < current_lower and hist.iloc[-1] > 0:
...
def identify_significant_swings(df, window=60, min_swing_size=0.05):
    """Mengidentifikasi swing signifikan"""
    highs = df['High']
    lows = df['Low']
...
# --- FUNGSI UTAMA APP ---
def app():
    """Fungsi utama yang dipanggil oleh Streamlit Multi-Page"""
    ...
    if st.button("🔍 Mulai Analisis"):
        with st.spinner('Sedang mengambil dan menganalisis data...'):
            ...
            fig.add_trace(go.Scatter(x=df.index, y=df['BB_Upper'], mode='lines', name='BB Upper', line=dict(color='red', width=1, dash='dot')))
            fig.add_trace(go.Scatter(x=df.index, y=df['BB_Middle'], mode='lines', name='BB Middle', line=dict(color='purple', width=1.5)))
            fig.add_trace(go.Scatter(x=df.index, y=df['BB_Lower'], mode='lines', name='BB Lower', line=dict(color='green', width=1, dash='dot'), fill='tonexty', fillcolor='rgba(173, 216, 230, 0.1)'))
            fig.add_trace(go.Scatter(x=df.index, y=np.where(df['Close'] > df['BB_Upper'], df['Close'], np.nan), mode='markers', name='Overbought', marker=dict(color='rgba(255, 0, 0, 0.6)', size=8, symbol='circle')))
            fig.add_trace(go.Scatter(x=df.index, y=np.where(df['Close'] < df['BB_Lower'], df['Close'], np.nan), mode='markers', name='Oversold', marker=dict(color='rgba(0, 255, 0, 0.6)', size=8, symbol='circle')))

            for i, level in enumerate(sr['Support']):
                fig.add_hline(y=level, line_dash="dash", line_color="green", annotation_text=f"Support {i+1}: Rp {level:,.2f}", annotation_position="bottom right")
            for i, level in enumerate(sr['Resistance']):
                fig.add_hline(y=level, line_dash="dash", line_color="red", annotation_text=f"Resistance {i+1}: Rp {level:,.2f}", annotation_position="top right")
            ...
            for key in fib_keys:
                if key in fib and not np.isnan(fib[key]):
                    ...
                    fig.add_hline(y=fib[key], line_dash=dash, line_color=color, annotation_text=f"{key.replace('Fib_', 'Fib ')}: Rp {fib[key]:,.2f}", annotation_position=position)

            if bb_analysis['squeeze_detected']:
                fig.add_annotation(x=df.index[-1], y=df['Close'].iloc[-1], text="⚠️ BOLLINGER SQUEEZE", showarrow=True, arrowhead=1, arrowsize=1, arrowwidth=2, arrowcolor="purple", font=dict(size=12, color="purple", weight="bold"), yshift=20)

            fig.update_layout(title=f"{ticker} - Analisa Teknikal Lengkap", xaxis_title="Tanggal", yaxis_title="Harga (Rp)", xaxis_rangeslider_visible=False, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), template="plotly_white", height=700, hovermode='x unified')
            st.plotly_chart(fig, use_container_width=True)

            # --- TAMPILKAN METRIK ---
            st.subheader("💰 Informasi Harga Terkini")
            col_prev, col_curr, col_change, col_vol = st.columns(4)
            with col_prev: st.metric("Harga Penutupan Hari Sebelumnya", f"Rp {previous_close:,.2f}")
            with col_curr: st.metric("Harga Penutupan Hari Ini", f"Rp {df['Close'].iloc[-1]:,.2f}")
            with col_change: st.metric("Perubahan Harga", f"{price_change:.2f}%", delta=f"{price_change:.2f}%", delta_color="inverse" if price_change < 0 else "normal")
            with col_vol: st.metric("Volume Perdagangan", f"{df['Volume'].iloc[-1]:,.0f}")
            ...
            with col_sup:
                if sr['Support']:
                    ...
                    for i, s in enumerate(sr['Support'][:3]): st.markdown(f"{i+1}. Rp {s:,.2f}")
                else: st.markdown("Tidak ada level support yang teridentifikasi")
            with col_res:
                if sr['Resistance']:
                    st.markdown("**Resistance Terkuat:**")
                    for i, r in enumerate(sr['Resistance'][:3]): st.markdown(f"{i+1}. Rp {r:,.2f}")
                else: st.markdown("Tidak ada level resistance yang teridentifikasi")
            ...
            if fib_display:
                ...
                for i, (key, value) in enumerate(fib_display.items()):
                    cols[i].metric(key.replace('Fib_', 'Fib ').replace('_', ' '), f"{value:,.2f}", help=f"Level {key.replace('Fib_', '')}")
            st.markdown('</div>', unsafe_allow_html=True)

            st.subheader("📊 Analisis Bollinger Bands")
            if bb_analysis['position']: st.markdown(f"**Posisi Harga:** {bb_analysis['position']}")
            if bb_analysis['trend']: st.markdown(f"**Analisis Tren:** {bb_analysis['trend']}")
            if bb_analysis['squeeze']: st.markdown(f"**Kondisi Bandwidth:** {bb_analysis['squeeze']}")
            if bb_analysis['reversal']: st.markdown(f"**Sinyal Reversal:** {bb_analysis['reversal']}")
            col_bb1, col_bb2, col_bb3 = st.columns(3)
            with col_bb1: st.metric("Upper Band", f"Rp {df['BB_Upper'].iloc[-1]:,.2f}")
            with col_bb2: st.metric("Middle Band (MA20)", f"Rp {df['BB_Middle'].iloc[-1]:,.2f}")
            with col_bb3: st.metric("Lower Band", f"Rp {df['BB_Lower'].iloc[-1]:,.2f}")

            # --- KESIMPULAN ---
            st.subheader("📋 Kesimpulan Analisis Teknikal")
            conclusions = generate_conclusion(df, mfi_value, macd_signal, bb_analysis, vol_anomali, price_change, market_trend)
            for conclusion in conclusions:
                if "REKOMENDASI" in conclusion:
                    st.markdown(f"<div style='background-color: #f0f2f6; padding: 15px; border-radius: 10px; border-left: 4px solid #1f77b4; margin: 10px 0;'>{conclusion}</div>", unsafe_allow_html=True)
                else:
            ...
            for conclusion in conclusions:
                if "REKOMENDASI" in conclusion:
                    if "BULLISH KUAT" in conclusion: main_recommendation = "Bullish Kuat"
                    elif "BEARISH KUAT" in conclusion: main_recommendation = "Bearish Kuat"
                    elif "BIAS BULLISH" in conclusion: main_recommendation = "Bias Bullish"
                    elif "BIAS BEARISH" in conclusion: main_recommendation = "Bias Bearish"
                if "TINGKAT RISIKO" in conclusion:
                    if "TINGGI" in conclusion: risk_level = "Tinggi"
                    elif "RENDAH" in conclusion: risk_level = "Rendah"

            color = "#28a745" if "Bullish" in main_recommendation else "#dc3545" if "Bearish" in main_recommendation else "#6c757d"
            icon = "📈" if "Bullish" in main_recommendation else "📉" if "Bearish" in main_recommendation else "⏺️"
            ...
            st.info("⚠️ **Disclaimer**: Analisis ini hanya untuk tujuan edukasi dan bukan sebagai rekomendasi investasi. Harga saham bisa berubah sewaktu-waktu. Lakukan riset tambahan dan konsultasi dengan penasihat keuangan sebelum mengambil keputusan investasi.")

# --- EXPORT UNTUK MULTI-PAGE STREAMLIT ---
__all__ = ['app']
